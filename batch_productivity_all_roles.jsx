...
function generateData(role, batchSize, points = 480) {
  const data = [];
  const baseline = 100;
  ...
  for (let i = 0; i < points; i++) {
    ...
    if (i % cycle === 0 && !nearBreak) productivity -= depth;
...
export default function App() {
  ...
  const data = generateData(selectedRole, selectedBatch);

  const batchSizes = [30, 15, 5];
  const allData = {
    30: generateData(selectedRole, 30),
    15: generateData(selectedRole, 15),
    5: generateData(selectedRole, 5),
  };
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: "13px", color: "#888", marginBottom: "24px" }}>
          Each role has different processing speeds. Select a role to see how batch size affects their variance patterns.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", marginBottom: "24px" }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: "28px", fontWeight: 700, color: "#fff" }}>
              {roleConfig.depths[selectedBatch]}%
            </div>
            <div style={{ fontSize: "10px", color: "#555", marginTop: "4px" }}>
        ...
        <p style={{ fontSize: "11px", color: "#333", marginTop: "24px", textAlign: "center" }}>
          Dip frequency and depth vary by role processing speed. Dashed lines = scheduled breaks and lunch.
        </p>
      </div>
    </div>
  );
